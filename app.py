...
from agent.llm_router import llm_response
import registry
...
def main():
    ...
    # 注册 LLM 双模式相关路由（模式切换、人设、记忆）
    from agent.llm_router import setup_llm_routes
    setup_llm_routes(appasync)
    from agent.memory_store import setup_memory_routes
    setup_memory_routes(appasync)
    # 注册对话会话路由（按数字人连接 sessionid 持久化聊天记录）
    from agent.conversation_store import setup_conversation_routes, reconcile_orphans
    setup_conversation_routes(appasync)
    reconcile_orphans()  # 启动时回收孤儿会话：磁盘上有文件却不在列表里的
    # 注册说话人分离路由（实时判定 / 离线精修 / 声纹库）
    from agent.speaker_diarize import setup_speaker_routes
    setup_speaker_routes(appasync)
