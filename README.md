...
> ⚠️ **已知限制**：实时说话人识别基于 CAM++ 声纹模型，**准确率有限**，在以下情况容易误判——
> - **短句 / 频繁打断**：同一人的多句短话可能被拆成多个「说话人N」（向量不稳定）
> - **单人对话**：其实无需区分说话人，误判会干扰标注
> - **环境噪声 / 数字人回声**：可能把数字人自己的声音误判成某个说话人
>
> 建议：
> - **单人对话**用 **solo 模式**（无需说话人区分），避免误判干扰
> - **多人会议**场景请用较长、连续的语音；识别不准时可对临时说话人**命名注册**，或对整段音频做**离线聚类精修**（`speaker_diarize.py`）
> - 判断回声是否正常，可检查 `web/asr/main.js` 的 `GAIN` 与 `speaker_store.py` 的 `self_filter_threshold`
