...
class BaseAvatar:
    def __init__(self, opt):
        ...
        self.llm_generation = 0  # LLM 代次：flush_talk 时递增，llm_response 检查代次变化来判断打断
    ...
    def put_audio_frame(self, audio_chunk:NDArray[np.float32], datainfo:dict={}): # 16khz 20ms pcm
        ...
        # ── 累积数字人实际播放的 TTS 音频，用于自适应更新自身声纹 ──
        # 用数字人真实发出的声音注册/更新 role="avatar"，让回声被 is_self 命中，
        # 无需依赖外部参考音频文件（换音色/重录都能自适应）。后台线程执行，不阻塞播放。
        try:
            status = datainfo.get("status", "")
            buf = getattr(self, "_avatar_tts_buf", None)
            if buf is None:
                buf = []
                self._avatar_tts_buf = buf
            if len(audio_chunk) > 0:
                buf.append(np.asarray(audio_chunk, dtype=np.float32).ravel())
            if status == "end" and buf:
                audio = np.concatenate(buf)
                self._avatar_tts_buf = []
                if len(audio) >= self.sample_rate * 0.3:
                    # 降频：首次播放注册一次，之后每 AVATAR_VP_REFRESH 秒才刷新一次，
                    # 避免每句都做一次 GPU 声纹提取，减少与 ASR/TTS 争抢 GPU。
                    now = time.time()
                    last = getattr(self, "_avatar_vp_time", 0.0)
                    if last == 0.0 or (now - last) >= 60.0:
                        self._avatar_vp_time = now
                        def _update(aud):
                            try:
                                from agent.speaker_store import update_avatar_voiceprint
                                update_avatar_voiceprint(aud, self.sample_rate)
                            except Exception:
                                pass
                        Thread(target=_update, args=(audio,), daemon=True).start()
        except Exception:
            pass
    ...
    def flush_talk(self):
        ...
        if hasattr(self, 'asr') and hasattr(self.asr, 'flush_talk'):
            self.asr.flush_talk()
        # 递增 LLM 代次，让正在生成的 LLM 线程检测到代次变化而停止
    ...
    def is_speaking(self) -> bool:
        # 检查 speaking 标志（由渲染线程更新）
        if self.speaking:
            return True
        # 补偿：TTS 队列有数据或正在播放中，也算正在说话
        if hasattr(self, 'tts') and hasattr(self.tts, 'msgqueue') and hasattr(self.tts, 'state'):
            if not self.tts.msgqueue.empty():
                return True
            from tts.base_tts import State
            if self.tts.state == State.RUNNING:
                return True
        return False
